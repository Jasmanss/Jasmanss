...
# (icon file, label, brand colour; None means the brand is black, so light up in lime)
GROUPS = [
    ("Languages", [
...
<style>
  ...
  @keyframes edge {{ 0%, 12%, 100% {{ stroke: {LINE}; }} 4% {{ stroke: {LIME}; }} }}
...
for r, (group, items) in enumerate(GROUPS):
    y = TOP + r * ROW_H
    out.append(f'<text x="{PAD}" y="{y + TILE_H/2 + 5}" class="mono" font-size="13" letter-spacing="2.4" fill="{BONE}" fill-opacity=".45">{group.upper().replace("&", "&amp;")}</text>')
    out.append(f'<rect x="{PAD}" y="{y + TILE_H/2 + 16}" width="18" height="3" fill="{LIME}"/>')
    for c, (name, label, color) in enumerate(items):
        x = PAD + LABEL_W + c * (TILE_W + GAP)
        delay = f"{i * STEP:.2f}s"
        s = 34 / 24
        out.append(
            f'<g class="t" style="--c:{color or LIME}">'
            f'<rect x="{x}" y="{y}" width="{TILE_W}" height="{TILE_H}" rx="10" fill="{TILE}" stroke="{LINE}" style="animation-delay:{delay}"/>'
            f'<path transform="translate({x + TILE_W/2 - 17} {y + 20}) scale({s:.4f})" d="{icon_path(name)}" fill="{BONE}" style="animation-delay:{delay}"/>'
            f'<text x="{x + TILE_W/2}" y="{y + TILE_H - 18}" text-anchor="middle" class="mono" font-size="12" letter-spacing="1.2" fill="{BONE}" fill-opacity=".5" style="animation-delay:{delay}">{label}</text>'
            f'</g>')
        i += 1
